...
$content_s{'DX1749'}="\\begin{tabular}{|p{0.5\\textwidth}<{\\centering}|p{0.515\\textwidth}<{\\centering}|} %设置表格宽度
		...
		{\\zihao{6}\\color{white} \\hspace{12cm}\\\$result\\_seal\\_user\\_flag\\_12\\\$}\\\\
		{\\song\\zihao{-4}\\bfseries 检测者：}{\\zihao{6}\\color{white}\\\$result\\\_seal\\\_user\\\_flag\\\_23\\\$}
		{\\song\\zihao{-4}\\bfseries 审核者：}{\\zihao{6}\\color{white}\\\$result\\\_seal\\\_user\\\_flag\\\_42\\\$}
...
{
	$| = 1; my $i = 3; while($i){ print "自动提取已完成，请关闭程序($i秒后自动关闭)\r"; $i--; sleep(1); }
}
